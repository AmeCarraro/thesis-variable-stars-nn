...
# Define label column names and load label data from CSV
column_names_y = ["oid", "class"]
features_df_y = pd.read_csv(
    y_file,
    sep=',',
    header=0
)

# Merge features and labels on 'oid'
merged_df = pd.merge(features_df, features_df_y, on='oid', how='inner')
print(merged_df.columns.tolist())
print(merged_df.head(1))

# Remove columns containing "_g" and filter for specific classes
...
# Map class labels to integers and save the labels to a file
label_mapping = { 
    'CEP': 0,
    'RR': 1,
    'RRc': 2,
...
label_path = os.path.join(data_path, "labelTR_30_svm.txt")
filtered_df['Label'].to_csv(label_path, index=False, header=False)

# Drop 'oid', 'class', and 'Label' columns from the dataframe
filtered_df = filtered_df.drop(columns=['oid', 'class', 'Label'])

# Convert object type columns to float and replace NaNs with 0
for col in filtered_df.columns:
    if filtered_df[col].dtype == 'object':
        filtered_df[col] = filtered_df[col].str.replace(',', '.', regex=False).astype(float)
filtered_df = filtered_df.fillna(0)

# Impute missing values (zeros) in each feature using SVR
...
# Select specific features for the final output and save to files
feature_names = [
   "Multiband_period",
   "Period_band_r",
   "Harmonics_mag_2_r",
...
output_file = os.path.join(data_path, "featureTR_30_svm.txt")
