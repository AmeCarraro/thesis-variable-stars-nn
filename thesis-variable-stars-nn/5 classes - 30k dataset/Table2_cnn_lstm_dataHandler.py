...
# Column definitions as in the ZTF variables catalog
cols = [
...
# - Observations: Num_g and Num_r > 50
# - Fit quality: R^2_g and R^2_r > 0.3
# - False Alarm Probability: log(FAP_g) and log(FAP_r) < -3
...
df = df[["Per", "R21", "phi21", "T_0", "rmag", "Amp_r", "Type"]]
...
for fold, (train_index, test_index) in enumerate(kf.split(balanced_df), 1):
    ...
    train_df.to_csv(f"{data_path}inf_lstm_train_fold_{fold}.txt", index=False, sep=' ', na_rep='NaN')
    test_df.to_csv(f"{data_path}inf_lstm_test_fold_{fold}.txt", index=False, sep=' ', na_rep='NaN')
